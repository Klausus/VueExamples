...
const pokeData = ref({});
const {data:pokemon, loading, getData} = GetData();
const useFavoritos = useFavoritosStore();
const {add:addFav, findPokemonByName, findPokemonById} = useFavoritos;


// METHODS or FUNCTIONS ////////////////////////////////////////////////////////////
const getPkmData = async (pokeName) => {
    getData(apiUrl + pokeName);
};
const getPkmDataOld = async (pokeName) => {
    try {
        const {data} = await axios.get(apiUrl + pokeName);
        pokemon.value = data;
        formatPokeData(data);
...
// WATCHERS ////////////////////////////////////////////////////////////////////////
watch(pokemon, (currentData) => {
    formatPokeData(currentData);
});
        ...
        <div clas="container" v-if="pokeData.id!=null">
            <div class="card">
                ...
                <div class="card-footer text-center">
                    <button class="btn btn-outline-danger btn-lg m-2" :disabled="findPokemonById(pokeData.id)" @click="addFav(pokeData)">
                        Agregar a Favoritos
                    </button>
                </div>
            </div>  
